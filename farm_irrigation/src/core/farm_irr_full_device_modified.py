...
try:
    # 优先使用真实的实时水位API
    from src.api.waterlevel_api import fetch_waterlevels  # 期望签名：fetch_waterlevels(farm_id: str, unit: str = "mm")
    print("[DEBUG] 使用真实实时水位API")
except Exception:
    try:
        # 备选方案：使用模拟API
        from src.api.mock_waterlevel_api import fetch_waterlevels  # 期望签名：fetch_waterlevels(farm_id: str, unit: str = "mm")
...
def generate_multi_pump_scenarios(cfg: FarmConfig, min_fields_trigger: int = 1) -> Dict[str, Any]:
    ...
    try:
        ...
        # 添加调试信息
        print(f"[DEBUG] 多水泵方案分析: 总田块数={len(cfg.fields)}, 有水位数据={fields_with_wl}, 无水位数据={fields_without_wl}, 低于阈值={len(fields_below_threshold)}")
        ...
        if len(fields_below_threshold) < min_fields_trigger:
            return {
                ...
                "analysis": {
                    "total_fields": len(cfg.fields),
                    "fields_with_waterlevel": fields_with_wl,
                    "fields_without_waterlevel": fields_without_wl,
