...
def _load_config(config_path: str = "auto_config_params.yaml") -> Dict[str, Any]:
    ...
    if not config_file.exists():
        ...
        return {
            ...
            "default_water_levels": {"wl_low": 30.0, "wl_opt": 80.0, "wl_high": 140.0},
...
if __name__ == "__main__":
    ...
    # 添加命令行参数解析
    parser = argparse.ArgumentParser(description="生成灌溉系统配置")
    parser.add_argument('--segments', help='水路GeoJSON文件路径')
    parser.add_argument('--gates', help='阀门GeoJSON文件路径')
    parser.add_argument('--fields', help='田块GeoJSON文件路径')
    args = parser.parse_args()
    
    # 如果提供了命令行参数，直接使用
    if args.segments and args.gates and args.fields:
