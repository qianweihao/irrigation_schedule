...
| **modified_plan_path** | string | 总是返回 | **🆕 修改后的计划文件保存路径**，可用于后续接口调用或执行 |
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| plan_id | string | ❌ | **可选**，计划ID或文件路径。<br>**业务含义**: 不传或为空时，自动使用output目录中最新的灌溉计划文件。<br>**示例**: `irrigation_plan_20251105_214020.json` 或不传（使用最新文件） |
...
**接口说明**: 清除批次重新生成和计划生成接口的缓存数据，强制下次请求重新计算
...
| 场景 | 说明 |
|------|------|
| **强制重新生成** | 当修改了配置文件或田块数据后，需要强制重新计算而不是返回缓存结果 |
| **测试调试** | 开发测试时，需要清除缓存以验证新的计算逻辑 |
| **数据更新后** | 手动更新了水位数据或其他关键参数，需要立即生效 |
| **缓存过期** | 虽然系统有5分钟自动过期，但某些情况下需要立即清除 |

**注意事项**
- ⚠️ 清除缓存后，下次请求会重新计算，可能需要较长时间
- ✅ 清除操作是安全的，不会影响已保存的计划文件
- 💡 缓存机制默认5分钟自动过期，通常无需手动清除
...
| plan_id | 文件路径 | 生产: `"/app/output/irrigation_plan_*.json"`<br>本地: `"e:/irrigation_schedule/farm_irrigation/output/irrigation_plan_*.json"` | 生成计划接口返回 | 启动执行、优化、重新生成 |
