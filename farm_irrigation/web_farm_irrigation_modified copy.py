...
let valveLayer = (map ? L.geoJSON(null, {
  pointToLayer: (feat, latlng)=> L.circleMarker(latlng, {radius:5, color:'#333', fillColor:'#ffd43b', fillOpacity:1, weight:1})
}).addTo(map) : null);
...
function highlightGatesAtTime(plan, tHour){
  // 重置
  Object.values(gateIndexByGid).forEach(ly=> ly.setStyle && ly.setStyle({color:'#333', fillColor:'#ffd43b'}));
  ...
  (plan.steps || []).forEach(s=>{
    if (tHour >= s.t_start_h && tHour <= s.t_end_h){
      (s.commands || []).forEach(c=>{
        ...
        let isOpen = false;
        if (act === 'open') isOpen = true;            // 旧式开闸
        else if (act === 'set') isOpen = Number(c.value||0) > 0;  // 关键：按 set 的数值判断
        else if (act === 'close') isOpen = false;
        ly.setStyle({color:'#111', fillColor: isOpen ? '#16a34a' : '#ef4444'});
      });
    }
  });
}
