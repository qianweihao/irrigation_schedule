...
# 检查当前目录
if [ ! -f "src/core/pipeline.py" ]; then
    print_error "未找到src/core/pipeline.py文件"
    print_error "请确保在项目根目录中运行此脚本"
    exit 1
fi
...
quick_run() {
    ...
    echo
    $PYTHON_CMD -m src.core.pipeline --input-dir ./data/gzp_farm --output-dir ./data/output
}
...
config_run() {
    ...
    if [ -f "pipeline_config.yaml" ]; then
        ...
        echo
        $PYTHON_CMD -m src.core.pipeline --config pipeline_config.yaml
    else
...
custom_run() {
    ...
    # 构建命令
    cmd="$PYTHON_CMD -m src.core.pipeline --input-dir \"$input_dir\" --output-dir \"$output_dir\""
...
show_help() {
    ...
    echo
    echo "更多信息请查看: $PYTHON_CMD -m src.core.pipeline --help"
    echo
    read -p "按Enter键返回菜单..."
}
